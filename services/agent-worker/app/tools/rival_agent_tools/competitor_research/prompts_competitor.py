...
{{
    ...
    "brand": "brand name",
    "variants": [
        {{"name": "Color or Size variant name", "price_delta": null}}
    ],
...
- estimated_price: current listed price as a finite number in TRY; null if not found (never NaN or Infinity).
...
- rating: between 0.0 and 5.0; null if not found (never NaN or Infinity).
- review_count: integer; null if not found. Do NOT estimate or invent this value — set to null if not directly observed (never NaN).
- brand: manufacturer or seller brand; null if unavailable.
- variants: list of product variants with name and price_delta from base price; price_delta null if unknown (never NaN or Infinity).
